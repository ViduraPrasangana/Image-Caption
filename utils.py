        ...
        with h5py.File(os.path.join(output_folder, split + '_IMAGES_' + base_filename + '.hdf5'), 'a') as h:
            ...
            for i, path in enumerate(tqdm(impaths)):
                ...
                try:
                    ...
                    print(img)
                    print(grey)
                    print(imageio.imread(impaths[i]).shape)
                    print(impaths[i-1])
                    print(imageio.imread(impaths[i]))
...
def create_input_files_2 (dataset, image_folder, captions_per_image, min_word_freq, output_folder,
                       max_len=100):
    """
    Creates input files for training, validation, and test data.
    ...
    assert dataset in {'iu'}

    # Read Karpathy JSON
    with open("dataset/iu/train_data.json", 'r') as j:
        train_data = json.load(j)
    with open("dataset/iu/test_data.json", 'r') as j:
